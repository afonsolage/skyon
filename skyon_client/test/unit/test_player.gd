...
#	func test_when_playing_attack_animation_call_attacking_started_func() -> void:
...
#		pending("Don't know how to test this")
	...
	func test_when_facing_monster_get_interation_area_bodies_should_return_it() -> void:
		var monster = TestUtils.create_static_body_cube(Vector3(0, 0, -2), [], [1])
		self.add_child_autoqfree(monster)
		var body = monster.get_child(0)
	...
	func test_when_not_facing_monster_player_or_resource_get_interation_area_bodies_should_not_return_it() -> void:
		var monster = TestUtils.create_static_body_cube(Vector3(0, 0, -2), [], [0,4,5,6])
		self.add_child_autoqfree(monster)
